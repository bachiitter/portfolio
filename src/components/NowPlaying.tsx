...
export function NowPlaying() {
  ...
  const currentPlayingQuery = useSWR(`/api/v1/spotify/now-playing`, fetcher, { refreshInterval: 1000 * 10, revalidateOnFocus: false })
  const lastPlayedQuery = useSWR(`/api/v1/spotify/last-played`, fetcher, { refreshInterval: 1000 * 30 })
  ...
  return <><Image
    ...
    width={20}
    height={20}
    className={`rounded-full ${currentPlayingQuery.data?.is_playing
      ? "animate-spin-slow"
      : "contrast-[0.35]"
      }`}
  />
    {currentPlayingQuery.data?.is_playing ? (
      ...
      <p className="line-clamp-1">
        ...
        <a
          href={
            lastPlayedQuery.data?.items[0].track.external_urls.spotify
          }
          className="font-semibold"
        >
